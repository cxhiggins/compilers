...
let rec gen_stmt s exit_lab =
  match s with
      Skip -> NOP
    | Seq stmts -> SEQ (List.map (fun x -> gen_stmt x exit_lab) stmts)
    | Assign (v, e) ->
    ...
    | RepeatStmt (body, test) ->
        let lab1 = label () and lab3 = label () in SEQ [LABEL lab1; gen_stmt body exit_lab; gen_cond test lab3 lab1; LABEL lab3]
    | Exit -> SEQ [JUMP exit_lab]
    | LoopStmt (body) ->
        let lab1 = label () and lab2 = label () in SEQ [LABEL lab1; gen_stmt body lab2; JUMP lab1; LABEL lab2]
