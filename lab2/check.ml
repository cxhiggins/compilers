...
and expr_type e env = 
  match e.e_guts with
      Variable x -> 
        lookup_def x env
    (* Step 2 — Check that v and e are valid expressions with appropriate types *)
    | Sub (v, e) ->
        let tv = (check_expr v env) and check_array_type = (
            function 
              Array(_, pt) -> pt
        ...
        if (is_int_type (check_expr e env)) then check_array_type tv else failwith "subscripts not implemented"
    | Constant (n, t) -> t
    | Monop (w, e1) -> 
...
let rec check_stmt s env =
  match s with
    ...
    | Assign (lhs, rhs) ->
        ...
        (* Step 2 — Check that only simple types are being equated *)
        let not_simple_type = (
          function
            Integer | Boolean -> false
          | _                 -> true
        ) in if (not_simple_type ta || not_simple_type tb) then sem_error "assignment of non-simple types" []
    | Print e ->
