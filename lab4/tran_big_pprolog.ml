...
(* Part 3 — Multiplying by a constant *)
...
(* |log_off| -- return (closest log_2, offset) of argument *)
(* unused, but could be used to add more mul optimizations! *)
let log_off x = 
  let rec loop log pow =
    if pow = x then (log, 0) else
    let next_pow = log * 2 in
      if pow < x && next_pow > x then
        if x - pow <= next_pow - x then (log, x - pow) else (log + 1, x - next_pow)
      else loop (log + 1) next_pow in
  loop 0 1
...
let rec e_reg t r =
  ...
  match t with
    ...
    | <BINOP Times, t1, t2> ->
        ...
        gen_reg "mul" [r; v1; v2]
    (* Part 3 — 9.4 Multiplying by a Constant *)
    (* | <BINOP Times, t1, t2> ->
        (let v1 = e_reg t1 anyreg in
        try
          match t2 with <CONST n> ->
            try
              let k = exact_log2 n in
              binary "lsl" t1 (<CONST k>)
            with Not_found ->
            try
              let k = exact_log2 (n-1) in
              reserve_reg (reg_of v1);
              gen_reg "add" [r; v1; Shift (reg_of v1, k)]
            with Not_found ->
            try 
              let k = exact_log2 (n+1) in
              reserve_reg (reg_of v1);
              gen_reg "rsb" [r; v1; Shift (reg_of v1, k)]
...
and e_rand =
  ...
  function
      ...
      (* Part 2 — 9.3 Shifter Operands *)
    (* | <BINOP Lsl, t1, <CONST k>> when fits_immed k ->
        let v1 = e_reg t1 anyreg in
          Shift (reg_of v1, k) *)
    | t -> e_reg t anyreg
...
and e_addr =
  (* returns |Index| *)
  function
      (* Part 4 — 9.5 Going the other way *)
      (* <LOCAL n> when fits_offset n ->
        Index (R_fp, n) 
      (* Part 1 — 9.2 More Addressing Modes *)
