...
(* |stack_jump| -- generate a list of n LDNW 12 instructions *)
let rec stack_jump n =
  if n = 0 then [] else LDNW 12 :: stack_jump (n-1)
...
let gen_addr d =
  if d.d_level = 0 then
    ...
    let fdiff = !level - d.d_level in
      SEQ [LOCAL 0; SEQ (stack_jump fdiff); CONST d.d_off; OFFSET]    
...
let rec gen_expr =
  function
    ...
    | Call (p, args) ->
        let arg_seq = SEQ (List.map gen_expr (List.rev args)) and n = List.length args and p_def = get_def p in
        let fdiff = !level - p_def.d_level + 1 in
        let sl = SEQ [LOCAL 0; SEQ (stack_jump fdiff); CONST p_def.d_off; OFFSET] in
        SEQ [LINE p.x_line; arg_seq; sl; GLOBAL p_def.d_lab; PCALLW n]
